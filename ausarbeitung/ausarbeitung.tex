...
der eine Ansatz basiert auf \glqq klassischen\grqq\ Methoden der Computer Vision, das heißt auf händisch ausgewählten Merkmalen wie Mittelwert, Standardabweichung, Farb-Histogrammen und Kantenzählung, auf deren Grundlage dann ein k-nearest-neighbor-Klassifikator angewendet wird.
...
%TODO: Sicherstellen, dass Grau-Hist in der Tabelle tatsächlich 35% waren?
\begin{table}[h]
\centering
...
Die erreichte Genauigkeit von 51\% ist sehr nah an dem Ergebnis unseres klassischen Ansatzes dran. Allerdings handelte es sich bei den 36 Bildern, welche von den Testpersonen klassifiziert werden sollten um gut klassifizierbare Bilder. das bedeutet.
%TODO:^^^^^^^^^^^^Der Satz ist eig noch nicht zu Ende?? Sollte man hier noch reinschreiben, dass die Ergebnisse auch ganz anders sein hätten können bei der Studie? evtl in Richtung, man kann ja den Testies nicht zumuten, alle Bilder so wie die Maschine zuzuordnen? Oder dass bei einer anderen Auswahl der 36 Bilder das Ergebniss ganz anders aussehen könnte?
